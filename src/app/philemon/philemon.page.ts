...
@Component({
  ...
  templateUrl: './philemon.page.html',
  styleUrls: ['./philemon.page.scss'],
})
export class PhilemonPage implements OnInit {

  percentage = 0;
  buttonCount = 1; // 假設你有50個按鈕
  increment = 100 / this.buttonCount; //100除以每一個按鈕的數量
  buttonStates = Array(this.buttonCount).fill(false); // 初始化按鈕狀態為false
  pageIndex = 23; // 假設這是第一個子頁面
