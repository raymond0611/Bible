...
@Component({
  ...
  templateUrl: './habakkuk.page.html',
  styleUrls: ['./habakkuk.page.scss'],
})
export class HabakkukPage implements OnInit {
  ...
  loadState() {
    const subPageIndex = this.pageIndex; // 使用子頁面的索引來獲取對應的進度百分比
    const subPagePercentage = localStorage.getItem('subPagePercentage' + subPageIndex);
    if (subPagePercentage) {
      this.percentage = parseFloat(subPagePercentage);
    }
